...
(function (Drupal) {
  ...
  **/
  ...
  ;
  /**
   * Overloading messenger animations.
   */
  (function ($, Drupal, drupalSettings) {
    'use strict';
  
    Drupal.behaviors.hubberMessenger = {
      attach: function (context, settings) {
        $(window, context).once('hubber-messenger').on('load', function () {
          $('.alert-danger, .alert-warning').parents('#page .highlighted').css({'animation': 'none'});
        });
      },
    };
  })(jQuery, Drupal, drupalSettings);
  ...
  (function ($, Drupal, drupalSettings) {
    ...
    Drupal.behaviors.btnLoader = {
      attach: function (context) {
        ...
        // Remove loader if webform phone checking has negative result.
        $('form .js-webform-telephone-international').once('form-field-focus').on('focus', function () {
          ...
          $form.find('.cta-blocker').remove();
  
          // Remove ajax throbber 100 milliseconds later because it takes 100 milliseconds to appear.
          setTimeout(function () {
  ...
  **/
  ...
  (function ($, Drupal, drupalSettings) {
    'use strict';
    Drupal.behaviors.hubberFilterBlockModeAuto = {
      HFPPTInit_done: false,
      attach: function (context) {
        if (context != document) {
          return;
        }
        ...
        // Price filtering initial params.
        var nap = 0;
        var nb_place_prop_name
          = $('.js-hvsc-map').length > 0 ? 'data-contig-max' : 'data-nb-place';
        var filtered_categories_by_block;
        var filtered_categories_by_price_type;
        var init = false;
        var price_filtering = false;
        var quantity_filtering = false;
  
        // Quantity filtering initial params.
        var range_value_min;
        var range_value_max;
  
        // Extended filtering initial params.
        var sort = 0;
        var hideEmpty = false;
        var sortCats = function () {
          var categories = $('.js-list-categories');
          var sortList = Array.prototype.sort.bind(categories);
          sortList(function (a, b) {
            if (sort == 0) {
              // Sort by category weight.
              return (parseFloat($(a).attr('data-index')) - parseFloat($(b).attr('data-index')));
            }
  
            // Out of stock categories should be put at the very end of the list.
            let element_a = parseInt($(a).attr('data-nb-place'));
            let element_b = parseInt($(b).attr('data-nb-place'));
            if (element_a === 0 && element_b !== 0) {
              return 1;
            }
            else if (element_a !== 0 && element_b === 0) {
              return -1;
            }
  
            // Sort by category min price.
            element_a = parseInt($(a).attr('data-price-min'));
            element_b = parseInt($(b).attr('data-price-min'));
            if (element_a === element_b) {
              // Sort by category max price.
              element_a = parseInt($(a).attr('data-price-max'));
              element_b = parseInt($(b).attr('data-price-max'));
            }
            if (element_a === element_b) {
              // Sort by category weight.
              return (parseFloat($(a).attr('data-index')) - parseFloat($(b).attr('data-index')));
            ...
            return sort * (element_a - element_b);
          });
          $('.js-block-list-categories').append(categories);
        };
        sortCats();
  
        $(context).on('HFPPTInit', function () {
          // Categories filter initialization.
          if (self.HFPPTInit_done) {
            return;
          }
          self.HFPPTInit_done = true;
          $(this).trigger('HFPPTEvent');
        }).on('HFPPTEvent', function (e, noTriggerClick) {
          // Categories filter update.
          $(this).trigger('HFPPTUpdateFilters');
          // Synchronize map with filters changes.
          var ids = [];
          var $categories = $('.js-list-categories').filter(function() {
            return $(this).css('display') != 'none';
          });
          if (self.selected_category == undefined) {
            $categories.each(function () {
              ids.push($(this).attr('cat_id'));
            });
          }
          else {
            ids = self.selected_category;
          }
          $('.js-hvsc-map').trigger('HTATCFEventCategoriesFilter', [ids]);
          noTriggerClick = (typeof noTriggerClick !== 'undefined') ? noTriggerClick : false;
          if (noTriggerClick) {
            return;
          }
  
          // If only one category, it has specific display.
          if ($categories.length === 1) {
            $categories.trigger('click', [true]);
          }
        }).on('HFPPTUpdateFilters', function (e) {
          // Update quantity for filtering.
          $(this).trigger('HFPPTUpdateQuantity');
  
          // Update categories filters.
          $('.js-list-categories', this).each(function (e) {
            // Get number of available seats for this category.
            var nb = $(this).attr(nb_place_prop_name);
  
            // Get id of this category.
            var id = $(this).attr('cat_id');
  
            // Get unit price of each category for filtering.
            var priceMin = $(this).attr('data-price-min');
            var priceMax = $(this).attr('data-price-max');
  
            if (
              filtered_categories_by_block != undefined && filtered_categories_by_block.indexOf(id) == -1 ||
              filtered_categories_by_price_type != undefined && filtered_categories_by_price_type.indexOf(id) == -1 ||
              priceMin > range_value_max ||
              priceMax < range_value_min ||
              nap > nb && nb > 0 ||
              nb == 0 && hideEmpty
            ) {
              $(this).hide();
              ...
              // This category is selected by filters.
              $(this).show();
            }
          });
  
          // Get product form.
          var form = $('#commerce-product-ticketing-form');
  
          if (nap > 0) {
            // Keep quantity.
            form.attr('quantity', nap);
          }
          else {
            // No defined quantity.
            form.removeAttr('quantity');
          }
  
          // Keep the information about contiguity.
          if (!form.prop('contig')) {
            form.prop('contig', quantity_filtering && nb_place_prop_name == 'data-contig-max');
          }
  
          // Update price filtering button.
          if (price_filtering) {
            var formatted_amount_min = Drupal.behaviors.hubberNumberFormatter.formatPrice(range_value_min, drupalSettings.currency);
            var formatted_amount_max = Drupal.behaviors.hubberNumberFormatter.formatPrice(range_value_max, drupalSettings.currency);
            $('.js_price_filter')
              .addClass('selected')
              .html(formatted_amount_min + ' - ' + formatted_amount_max);
          }
          else {
            $('.js_price_filter')
              .removeClass('selected')
              .html(Drupal.t('Price /ticket'));
          }
        }).on('HFPPTUpdateQuantity', function () {
          ...
          if (nap > 0) {
            $('.js_quantity_filter')
              ...
              .html(
                nap == 1
                  ? Drupal.t('1 ticket')
                  : Drupal.t('%n tickets', {'%n': nap}));
  
            // Update price and tickets number  display.
            $('.js-categories-panel').each(function () {
              // Get price data.
              var blockPriceData = drupalSettings.hubberVirtualCart.catPriceDatas[$(this).attr('hvsc-id')];
  
              for (var catId in blockPriceData) {
                // Nothing to do if this category is excluded by price type
                // selection.
                if (
                  filtered_categories_by_price_type != undefined
                  && filtered_categories_by_price_type.indexOf(catId) == -1) {
                  continue;
                }
  
                var price = 0;
                for (var priceId in self.quantity_details) {
                  if (0 == self.quantity_details[priceId]) {
                    continue;
                  }
  
                  price
                    += self.quantity_details[priceId]
                    * blockPriceData[catId].type_prices[priceId].price;
                }
  
                // Update price for the category.
                $('.js-list-categories[cat_id=' + catId + ']', this)
                  .attr('data-current-price', price);
              }
            });
          }
          else {
            // If quantity is undefined, express mode is impossible.
            $('.js-category-auto').prop('disabled', true);
  
            $('.js_quantity_filter')
              .removeClass('selected')
              .html(Drupal.t('Quantity'));
  
            // Reset price for each category.
            $('.js-list-categories').removeAttr('data-current-price');
          }
        }).on('HFPPTPriceEvent', function (e, hfppt_range_value_min, hfppt_range_value_max) {
          // After validation, hide the form.
          $('#filter-form-price').hide().parents('.filter').removeClass('active');
          $('#filter-form-type-price').hide().parents('.filter').removeClass('active');
  
          range_value_min = parseInt(hfppt_range_value_min);
          range_value_max = parseInt(hfppt_range_value_max);
          price_filtering = true;
          $(this).trigger('HFPPTEvent');
        }).on('HFPPTQuantityEvent', function (e, requested_quantity, hfppt_contig, hfppt_categories) {
  
          // After validation, hide the form.
          $('#filter-form-quantity').hide().parents('.filter').removeClass('active');
  
          quantity_filtering = true;
  
          // Save the total quantity.
          $('.js-list-categories').attr('data-qty', requested_quantity);
          nap = requested_quantity;
  
          // If there is a map, user can select places by contiguity.
          if ($('.js-hvsc-map').length > 0) {
            nb_place_prop_name = hfppt_contig ? 'data-contig-max' : 'data-nb-place';
          }
  
          // Get categories filter by price type.
          filtered_categories_by_price_type
            = hfppt_categories == null ? undefined : hfppt_categories;
  
          $(this).trigger('HFPPTEvent');
        }).on('HFPPTExtendedFilterEvent', function (e, hfppt_sort, hfppt_hide) {
          // After validation, hide the form.
          $('#filter-form-extended').hide().parents('.filter').removeClass('active');
          $('#filter-form-extended-mobile').hide();
  
          // Extended filter changed.
          hideEmpty = hfppt_hide;
          $(this).trigger('HFPPTEvent');
          sort = hfppt_sort;
          sortCats();
        }).on('HFPPTPriceLoadForm', function () {
          // Show only price filter form.
          $('.ticket-filter-form').hide().parents('.filter').removeClass('active');
          $('#filter-form-price').show().parents('.filter').addClass('active');
          $('.ticket-content').css('overflow', 'visible');
          $('#filter-form-type-price').hide().parents('.filter').removeClass('active');
        }).on('HFPPTTypePriceLoadForm', function () {
          // Show only type price filter form.
          $('.ticket-filter-form').hide().parents('.filter').removeClass('active');
          $('#filter-form-price').hide().parents('.filter').removeClass('active');
          $('#filter-form-type-price').show().parents('.filter').addClass('active');
        }).on('HFPPTQuantityLoadForm', function () {
          // Contiguity option can be used if there is a map.
          if ($('.js-hvsc-map').length > 0) {
            $('input[name=contig]')
              .prop('checked', nb_place_prop_name == 'data-contig-max');
          }
  
          // Show only quantity filter form.
          $('.ticket-filter-form').hide().parents('.filter').removeClass('active');
          $('#filter-form-quantity').show().parents('.filter').addClass('active');
          $('.ticket-content').css('overflow', 'visible');
        }).on('HFPPTExtendedLoadForm', function () {
          // Show only extended filter form.
          $('.ticket-filter-form').hide().parents('.filter').removeClass('active');
          $('#filter-form-extended').show().parents('.filter').addClass('active');
        }).on('reset', function () {
          // Go back to initial filter params.
          nap = 0;
          self.quantity_details = {};
          nb_place_prop_name = 'data-contig-max';
          range_value_min = undefined;
          range_value_max = undefined;
          filtered_categories_by_price_type = undefined;
          ...
          // Reset every filter form.
          var slider = $('.js-hubber-slider');
          var input = slider.closest('.filter-price-element').find('.hubber-slider-input');
          slider.slider('option', {
            values: [parseInt(input.attr('data-min')), parseInt(input.attr('data-max'))]
          });
  
          // Set quantity fields to O.
          $('.js-nb-tickets-selector').val($('.js-nb-tickets-selector').attr('data-default-value')).trigger('change');
          $('.input-number-operators').trigger('change', 0);
          $('.js-list-categories').attr('data-qty', 1);
  
          $('select[name=sort]').val(-1).trigger('change');
          $('input[name=hide]').prop('checked', false);
        }).on('HFPPTSelectCategory', function (e, catId) {
          // When user selects a category.
          self.selected_category = [catId];
          $(this).trigger('HFPPTEvent', [true]);
          $('.js-hvsc-map').trigger('HTATCFSelectCategory', [catId]);
        }).on('HFPPTUnSelectCategory', function () {
          // When user unselect the category.
          self.selected_category = undefined;
          $(this).trigger('HFPPTEvent');
          $('.js-hvsc-map').trigger('HTATCFUnSelectCategory');
        }).on('HFPPTSelectCategories', function (e, blockPriceData, blockId) {
          // Filter categories by block selected.
          filtered_categories_by_block = Object.keys(blockPriceData);
  
          // Set block id on panel.
          $('.js-categories-panel').attr('block-id', blockId);
  
          $(this).trigger('HFPPTUpdateFilters');
        }).on('HFPPTUnSelectCategories', function () {
          // Remove block id on panel.
          $('.js-categories-panel').removeAttr('block-id');
  
          filtered_categories_by_block = undefined;
          self.selected_category = undefined;
          $(this).trigger('HFPPTEvent');
        });
  
        // Open promotional code popin at the beginning.
        if (typeof drupalSettings.forcePromoFormOpening !== 'undefined' && drupalSettings.forcePromoFormOpening) {
          $(document).trigger('HFPPTTypePriceLoadForm');
        }
  
        // close filter wrapper when click outside
        $(document).once('mousedown-init').on('mousedown', function (e) {
          var $container = $('.btn-filter, .filter-wrapper'); // selecteur de la popin ouverte
          if (!$container.is(e.target) && $container.has(e.target).length === 0) {
            $('.filter.active').find('.filter-wrapper').hide();
            $('.filter.active').removeClass('active');
            $('.ticket-content').css('overflow', '');
          }
        });
  
        $(document).once('map-resize').on('mapResize', function (e) {
          if ($('.plan').hasClass('full')) {
            $('.ticket-content-wrapper').addClass('only-map');
          }
          else {
            $('.ticket-content-wrapper').removeClass('only-map');
          }
        });
  
        $(document).once('map-hidden').on('mapHidden', function (e) {
          $('.ticket-content-wrapper').removeClass('only-map');
        });
  
        // When user clicks on quantity filter button.
        $('.js_quantity_filter').click(function () {
          $('.js-block-list-categories').trigger('HFPPTQuantityLoadForm');
        });
  
        // When user clicks on price filter button.
        $('.js_price_filter').click(function () {
          $('.js-block-list-categories').trigger('HFPPTPriceLoadForm');
        });
  
        // When user clicks on type price filter button.
        $('.js_type_price_filter').click(function () {
          $('.js-block-list-categories').trigger('HFPPTTypePriceLoadForm');
        });
  
        // When user clicks on extended filter button.
        $('.btn-more-filters').click(function () {
          $('.js-block-list-categories').trigger('HFPPTExtendedLoadForm');
        });
  
        // When user clicks on quantity filter form cancel button.
        $('#filter-form-quantity .js-cancel').click(function (e) {
          e.preventDefault();
          $('#filter-form-quantity').hide().parents('.filter').removeClass('active');
        });
  
        // When user clicks on quantity filter form validation button.
        $('#hubber-filter-quantity-ticketing-form .js-submit').click(function (e) {
          e.preventDefault();
          var prices = [];
          var catPriceDatas = drupalSettings.hubberVirtualCart.catPriceDatas[$(this).attr('hvsc-id')];
  
          const requested_value = parseInt($('#hubber-filter-quantity-ticketing-form .js-nb-tickets-selector').val());
          $('.js-list-categories').each(function () {
            // Update the quantity if there is only one rate in this category.
            var $typePriceInputs = $(this).find('.input-number-operators');
            if ($typePriceInputs.length === 1) {
              $typePriceInputs.trigger('change', requested_value);
            }
          });
  
          $('.js-block-list-categories').trigger('HFPPTQuantityEvent', [
            requested_value,
            $('#edit-contig').val()
          ]);
        });
  
        // When user clicks on price filter form cancel button.
        $('#hubber-filter-price-ticketing-form .js-cancel').click(function (e) {
          e.preventDefault();
          $('#filter-form-price').hide().parents('.filter').removeClass('active');
        });
  
        // When user clicks on type price filter form cancel button.
        $('#hubber-filter-type-price-ticketing-form .js-cancel, #hubber-filter-type-price-ticketing-form .js-submit').click(function (e) {
          e.preventDefault();
          $('#filter-form-type-price').hide().parents('.filter').removeClass('active');
        });
  
        // When user clicks on price filter form validation button.
        $('#hubber-filter-price-ticketing-form .js-submit').click(function (e) {
          e.preventDefault();
          $('.js-block-list-categories')
            .trigger(
              'HFPPTPriceEvent',
              $('#hubber-filter-price-ticketing-form').find('[name=slider_range_price]').val().split(','));
        });
  
        // When user clicks on extended filter form cancel button.
        $('.ticket-filter-form .js-cancel').click(function (e) {
          e.preventDefault();
          $('.ticket-filter-form').hide().parents('.filter').removeClass('active');
        });
  
        // When user clicks on extended filter form submit button.
        $('.hubber-filter-extended-ticketing-form .js-submit').click(function (e) {
          e.preventDefault();
          self.processExtendedForm();
        });
  
        // When user clicks on extended filter form reset button.
        $('.hubber-filter-extended-ticketing-form .js-reset').click(function (e) {
          e.preventDefault();
          $('.ticket-filter-form').hide().parents('.filter').removeClass('active');
          $('.js-list-categories').trigger('reset');
        });
  
        $('.filter .js-reset, .filter .js-cancel, .filter .js-submit').once('filter-actions-click').on('click', function () {
          $('.ticket-content').css('overflow', '');
        });
      },
      quantity_details: {},
  
      processExtendedForm: function () {
        const form = $('#hubber-filter-extended-ticketing-form');
        $('.js-block-list-categories').trigger('HFPPTExtendedFilterEvent', [
          $('[name=sort]', form).val(),
          $('[name=hide]', form).prop('checked'),
        ]);
        $('.js-block-list-categories')
          .trigger('HFPPTPriceEvent', $('[name=slider_range_price]', form).val().split(','));
      }
    };
  })(jQuery, Drupal, drupalSettings);
  ;
  // Display fullscreen loader until the page is fully loaded.
  var checkReadyState = setInterval(function() {
    if (document.readyState === 'complete') {
      clearInterval(checkReadyState);
      Drupal.behaviors.hubberLoader.hideFullscreenLoader();
    }
  }, 100);
   ...
   *
   *This library manages the sales rules for the malp.
   *
   * --------------------------
   *    SPECIAL JS VARIABLE
   * --------------------------
   ...
   *        #plugin_id_qty# => [ // #plugin_id_qty# in
   *   'CHECK_MIN_QTY','CHECK_MAX_QTY','CHECK_EQUAL_QTY','CHECK_MULTIPLE_QTY',
   *          'variation' => [
   *            #variation_id# => #valeur de la quantit̩#,
   *          ],
   *          'variation_by_order' => [
   *            #variation_id# => #valeur de la quantit̩#,
   *          ],
   ...
   *          'attribute' => [
   *            #attribute_name => [
   *              #atribute_id# => #valeur de la quantit̩#,
   *            ],
   *          ],
   *          'attribute_by_order' => [
   *            #attribute_name => [
   ...
   *          ],
   *          'provider' => [
   *            #provider_id# => #valeur de la quantit̩#,
   *          ],
   *          'provider_by_order' => [
   *            #provider_id# => #valeur de la quantit̩#,
   *          ],
   ...
   *            #provider_id# => #valeur de la quantit̩#,
   *          ],
   *          'all'=> #valeur de la quantit̩#,
   *            #variation_id# => #valeur de la quantit̩#,
   *          ],
   *        ],
   *        #plugin_id_rate# => [ // #plugin_id_rate# is 'CHECK_UNLOCKED_TICKET',
   *   'CHECK_UNLOCKED_TICKET',
   *          #product_id# => [
   *            'parent' => [
   ...
   *     This event is triggered when a seat is added in VSCart
   ...
   *     This event is triggered when a seat is removed in VSCart
   *     @param {Integer} hvsc_id
   *       Id of hubberVirtualSeatCart instance that have trigger the event.
   *     @param {Object} seat
   *       The seat object that has been seat has been removed from the vscart.
   *
   *   - VSCartSeatRefreshDisplayDetailEvent
   *     This event is triggered when a VSCart is updated
   *     @param {Integer} hvsc_id
   *       Id of hubberVirtualSeatCart instance that have trigger the event.
   *     @param {String} detail
   *       The HTML code of the VSCart.
   *
   */
  
  (function ($, Drupal, drupalSettings) {
    'use strict';
    Drupal.behaviors.hubberSaleRulesForMalp = {
      attach: function (context) {
        var self = this;
  
        //Notify the user if he is entitled to a blocked tariff
        $('.js-hvsc-vscart', context).on('VSCartSeatAddedEvent', function (e, hvsc_id, seat) {
          var cart = Drupal.behaviors.hubberVirtualCart.getVSCart(hvsc_id);
          var totalTypePricesInVSCart = self.getTypePriceInVSCart(hvsc_id, cart);
          var available_prices = [];
          $(seat.all_prices).each(function () {
            var type_price_id = this;
            if (self.testIfSaleRuleRateExist('CHECK_UNLOCKED_TICKET', seat.product_id, 'child', type_price_id)) {
              var sale_rules = drupalSettings.saleRules['CHECK_UNLOCKED_TICKET'][seat.product_id]['child'][type_price_id];
              $(sale_rules).each(function () {
                let total_parent_rate = 0;
                for (let j in this.parent_rate) {
                  let parent_rate_id = this.parent_rate[j];
                  total_parent_rate += totalTypePricesInVSCart[parent_rate_id] !== undefined ? parseInt(totalTypePricesInVSCart[parent_rate_id]) : 0
                }
                var nb_rate_unlocked = Math.floor(total_parent_rate / this.parent_qty) * this.child_qty;
  
                //delete child rate present in basket
                if (hubberSaleRulesProduct.testIfRateIsInArray(drupalSettings.dataBasketForSaleRules, seat.product_id, type_price_id)) {
                  nb_rate_unlocked = nb_rate_unlocked - drupalSettings.dataBasketForSaleRules['attributes_by_product'][seat.product_id]['attribute_type_price'][type_price_id];
                }
  
                //delete child rate present in past order
                if (this.has_sale_rule_by_user && hubberSaleRulesProduct.testIfRateIsInArray(drupalSettings.dataPastOrderForSaleRules, seat.product_id, type_price_id)) {
                  nb_rate_unlocked = nb_rate_unlocked - drupalSettings.dataPastOrderForSaleRules['attributes_by_product'][seat.product_id]['attribute_type_price'][type_price_id];
                }
  
                var total_parent_child = totalTypePricesInVSCart[type_price_id] !== undefined ? totalTypePricesInVSCart[type_price_id] : 0;
                nb_rate_unlocked -= total_parent_child;
                if (nb_rate_unlocked > 0) {
                  available_prices.push(type_price_id);
                }
              });
            }
            else {
              available_prices.push(this);
            }
  
          });
          seat.available_prices = available_prices;
          for (var i in cart) {
            var seat_in_cart = cart[i];
            if (seat_in_cart.hubber_seat_id == seat.hubber_seat_id) {
              cart[i] = seat;
            }
          }
          Drupal.behaviors.hubberVirtualCart.updateVSCart(hvsc_id, cart, seat);
        });
  
        $('.js-hvsc-vscart', context).on('VSCartSeatRemovedEvent', function (e, hvsc_id, seat) {
          var $myDialog = '';
          var cart = Drupal.behaviors.hubberVirtualCart.getVSCart(hvsc_id);
          var totalTypePricesInVSCart = self.getTypePriceInVSCart(hvsc_id, cart);
          var rate_disabled = false;
          // deletion of a parent rate
          if (self.testIfSaleRuleRateExist('CHECK_UNLOCKED_TICKET', seat.product_id, 'parent', seat.type_price_id)) {
            var sale_rules = drupalSettings.saleRules['CHECK_UNLOCKED_TICKET'][seat.product_id]['parent'][seat.type_price_id];
            $(sale_rules).each(function () {
              let total_parent_rate = 0;
              for (let j in this.parent_rate) {
                let parent_rate_id = this.parent_rate[j];
              ...
              var total_child_rate = totalTypePricesInVSCart[this.child_rate] !== undefined ? totalTypePricesInVSCart[this.child_rate] : 0;
              var total_rate = parseInt(total_parent_rate) + parseInt(total_child_rate);
              var sale_rule_qty = parseInt(this.parent_qty) + parseInt(this.child_qty);
              var rest = total_rate % sale_rule_qty;
              var nb_rate_child_allowed = Math.floor(total_rate / sale_rule_qty) * this.child_qty;
              if ((rest - this.parent_qty) > 0) {
                nb_rate_child_allowed += parseInt((rest - this.parent_qty) * this.child_qty);
              }
  
              var child_rate = this.child_rate;
              let nb_rate_child_seats = 0;
              for (var i in cart) {
                var seatVSCart = cart[i];
                if ($.inArray(child_rate, seatVSCart.available_prices) > -1) {
                  nb_rate_child_seats++;
                  if (nb_rate_child_seats > nb_rate_child_allowed) {
                    Drupal.behaviors.hubberVirtualCart.removeFromVSCart(hvsc_id, seatVSCart.hubber_seat_id);
                    rate_disabled = true;
                  }
                }
              }
              ...
              if (rate_disabled) {
                $myDialog = $('<div class="css-popin-info">' + Drupal.t("The deletion of this ticket has resulted in the removal of a ticket of price %label", {'%label': this.child_label}) + '</div>').appendTo('body');
                Drupal.dialog($myDialog).showModal();
              }
            });
          }
  
          if (rate_disabled == false) {
            $(seat.available_prices).each(function () {
              var rate_id = this;
              //deletion of child rate
              if (self.testIfSaleRuleRateExist('CHECK_UNLOCKED_TICKET', seat.product_id, 'child', rate_id)) {
                var sale_rules = drupalSettings.saleRules['CHECK_UNLOCKED_TICKET'][seat.product_id]['child'][rate_id];
                $(sale_rules).each(function () {
                  var rest = self.getRestRate(seat.type_price_id, this, totalTypePricesInVSCart, seat.product_id);
                  if (rest > this.parent_qty || rest == 0) {
                    var nb_option_to_add = rest == 0 ? 1 : rest - this.parent_qty;
                    var i = 0;
                    for (var i in cart) {
                      var seatVSCart = cart[i];
                      if (i < nb_option_to_add && $.inArray(rate_id, seatVSCart.available_prices) == -1) {
                        seatVSCart.available_prices.push(rate_id);
                        cart[i] = seatVSCart;
                        i++;
                      }
                    }
          ...
          Drupal.behaviors.hubberVirtualCart.updateVSCart(hvsc_id, cart);
        });
  
        $('.js-hvsc-vscart', context).once('hvsc-map-seat-deselected').on('VSCartSeatRefreshDisplayDetailEvent', function (e, hvsc_id, detail) {
          self.displayErrorMessage(hvsc_id);
          self.displayUnlockedPriceMessage(hvsc_id, hvsc_id);
        });
  
        $('.js-hvsc-vscart', context).once('hvsc-map-seat-price-updated').on('VSCartSeatPriceUpdated', function (e, hvsc_id, seat) {
          self.displayErrorMessage(hvsc_id);
          self.displayUnlockedPriceMessage(hvsc_id, seat.product_id);
        });
  
        $('.js-hvsc-vscart', context).once('hvsc-map-seat-display-vscart').on('HTATCFEventSelectMALP', function (e, hvsc_id, detail) {
          self.displayErrorMessage(hvsc_id);
        });
      },
  
      displayUnlockedPriceMessage: function (hvsc_id, product_id) {
        var self = this;
        $('.data-info-msg').remove();
        for (var type_offer of ['CHECK_UNLOCKED_TICKET', 'CHECK_COUPLED_TICKET']) {
          if (drupalSettings.saleRules[type_offer] &&
            drupalSettings.saleRules[type_offer][product_id] &&
            drupalSettings.saleRules[type_offer][product_id]['child']) {
  
            var cart = Drupal.behaviors.hubberVirtualCart.getVSCart(hvsc_id);
            var totalTypePricesInVSCart = self.getTypePriceInVSCart(hvsc_id, cart);
            var sale_rules = drupalSettings.saleRules[type_offer][product_id]['child'];
            for (var type_price_id in sale_rules) {
              $(sale_rules[type_price_id]).each(function () {
                let total_parent_rate = 0;
                for (let j in this.parent_rate) {
                  let parent_rate_id = this.parent_rate[j];
                  total_parent_rate += totalTypePricesInVSCart[parent_rate_id] !== undefined ? parseInt(totalTypePricesInVSCart[parent_rate_id]) : 0
                ...
                //delete child rate present in basket
                if (hubberSaleRulesProduct.testIfRateIsInArray(drupalSettings.dataBasketForSaleRules, product_id, type_price_id)) {
                  nb_rate_unlocked = nb_rate_unlocked - drupalSettings.dataBasketForSaleRules['attributes_by_product'][product_id]['attribute_type_price'][type_price_id];
                }
  
                //delete child rate present in past order
                if (this.has_sale_rule_by_user && hubberSaleRulesProduct.testIfRateIsInArray(drupalSettings.dataPastOrderForSaleRules, product_id, type_price_id)) {
                  nb_rate_unlocked = nb_rate_unlocked - drupalSettings.dataPastOrderForSaleRules['attributes_by_product'][product_id]['attribute_type_price'][type_price_id];
                }
  
                var total_parent_child = totalTypePricesInVSCart[type_price_id] !== undefined ? totalTypePricesInVSCart[type_price_id] : 0;
                nb_rate_unlocked -= total_parent_child;
                if (nb_rate_unlocked > 0) {
                  var message = Drupal.formatPlural(nb_rate_unlocked, 'Your selection allows you to add 1 additional ticket of price %label', 'Your selection allows you to add @count additional tickets of price %label', {'%label': this.child_label});
                  $('.js-hvsc-vscart .js-hvsc-vscart-detail').append('<div class="data-info-msg"><span>' + message + '</span></div>');
                }
              });
            }
      ...
      /**
       * Return the number of ticket by rate id
       *
       * @param {Object} cart The vscart object.
       * @returns {Array} Array of the number of tickets indexed by rate id
       */
      getTypePriceInVSCart: function (id_product, cart) {
        var totalTypePricesInVSCart = [];
        for (var i in cart) {
          var seat = cart[i];
          if (!seat.has_defined_type_price) {
            continue;
          }
          if (totalTypePricesInVSCart[seat.type_price_id] === undefined) {
            totalTypePricesInVSCart[seat.type_price_id] = 1;
          }
          else {
            totalTypePricesInVSCart[seat.type_price_id]++;
          }
        }
        this.addTotalRateInRealCart(totalTypePricesInVSCart, id_product, drupalSettings.dataBasketForSaleRules);
        return totalTypePricesInVSCart;
      },
  
      /**
       * Calculates the number of tickets remaining after using a sales rule
       *
       * @param {Integer} type_price_id
       * @param {Object} sale_rule
       * @param {Array} totalTypePricesInVSCart
       * @returns {Number}
       */
      getRestRate: function (type_price_id, sale_rule, totalTypePricesInVSCart, id_product) {
        var temporary_totalTypePricesInVSCart = totalTypePricesInVSCart;
        if (sale_rule.has_sale_rule_by_user) {
          temporary_totalTypePricesInVSCart = this.addPastOrderForSaleRule(temporary_totalTypePricesInVSCart, id_product, drupalSettings.dataPastOrderForSaleRules);
        }
  
        let total_parent_rate = 0;
        for (let j in sale_rule.parent_rate) {
          let parent_rate_id = sale_rule.parent_rate[j];
          total_parent_rate += temporary_totalTypePricesInVSCart[parent_rate_id] !== undefined ? parseInt(temporary_totalTypePricesInVSCart[parent_rate_id]) : 0
        }
  
        var total_child_rate = temporary_totalTypePricesInVSCart[type_price_id] !== undefined ? temporary_totalTypePricesInVSCart[type_price_id] : 0;
  
        var total_rate = parseInt(total_parent_rate) + parseInt(total_child_rate);
        var sale_rule_qty = parseInt(sale_rule.parent_qty) + parseInt(sale_rule.child_qty);
        return total_rate % sale_rule_qty;
      },
  
      /**
       * Tests if a rate belongs to a sales rule
       *
       ...
       * @returns {Boolean}
       */
      testIfSaleRuleRateExist: function (plugin_id, product_id, type, rate_id) {
        if (drupalSettings.saleRules[plugin_id] &&
          drupalSettings.saleRules[plugin_id][product_id] &&
          drupalSettings.saleRules[plugin_id][product_id][type] &&
          drupalSettings.saleRules[plugin_id][product_id][type][rate_id]) {
          return true;
        }
        return false;
      },
      ...
      /**
       * Returns the total of tickets for sales rules of type quantity
       *
       * @param {Object} cart
       * @returns {Array}
       */
      getTotalInVSCart: function (cart, has_by_user) {
        var total = hubberSaleRulesProduct.initTotal(has_by_user);
  
        for (var i in cart) {
          var seat = cart[i];
  
          total['total_basket'] = total['total_basket'] === 0 ? 1 : parseInt(total['total_basket']) + parseInt(1);
          total['total_all'] = total['total_all'] === 0 ? 1 : parseInt(total['total_all']) + parseInt(1);
          total['total_by_provider'][seat.provider_id] = total['total_by_provider'][seat.provider_id] === undefined ? 1 : parseInt(total['total_by_provider'][seat.provider_id]) + parseInt(1);
          total['total_by_type']['ticketing'] = total['total_by_type']['ticketing'] === undefined ? 1 : parseInt(total['total_by_type']['ticketing']) + parseInt(1);
          total['total_by_category'][seat.category_id] = total['total_by_category'][seat.category_id] === undefined ? 1 : parseInt(total['total_by_category'][seat.category_id]) + parseInt(1);
          total['total_by_type_price'][seat.type_price_id] = total['total_by_type_price'][seat.type_price_id] === undefined ? 1 : parseInt(total['total_by_type_price'][seat.type_price_id]) + parseInt(1);
          if (total['total_by_category_by_product'][seat.product_id] === undefined) {
            total['total_by_category_by_product'][seat.product_id] = [];
          }
          if (total['total_by_type_price_by_product'][seat.product_id] === undefined) {
            total['total_by_type_price_by_product'][seat.product_id] = [];
          }
          total['total_by_category_by_product'][seat.product_id][seat.category_id] = total['total_by_category_by_product'][seat.product_id][seat.category_id] === undefined ? 1 : parseInt(total['total_by_category_by_product'][seat.product_id][seat.category_id]) + parseInt(1);
          total['total_by_type_price_by_product'][seat.product_id][seat.type_price_id] = total['total_by_type_price_by_product'][seat.product_id][seat.type_price_id] === undefined ? 1 : parseInt(total['total_by_type_price_by_product'][seat.product_id][seat.type_price_id]) + parseInt(1);
  
          total['total_by_product'][seat.product_id] = total['total_by_product'][seat.product_id] === undefined ? 1 : parseInt(total['total_by_product'][seat.product_id]) + parseInt(1);
          total['total_by_variation'][seat.variation_id] = total['total_by_variation'][seat.variation_id] === undefined ? 1 : parseInt(total['total_by_variation'][seat.variation_id]) + parseInt(1);
          if (total['type_price_by_category'][seat.category_id] === undefined) {
            total['type_price_by_category'][seat.category_id] = [];
          }
          total['type_price_by_category'][seat.category_id][seat.type_price_id] = total['type_price_by_category'][seat.category_id][seat.type_price_id] === undefined ? 1 : parseInt(total['type_price_by_category'][seat.category_id][seat.type_price_id]) + parseInt(1);
  
          total['total_all_by_user'] = total['total_all_by_user'] === 0 ? 1 : parseInt(total['total_all_by_user']) + parseInt(1);
          total['total_by_provider_by_user'][seat.provider_id] = total['total_by_provider_by_user'][seat.provider_id] === undefined ? 1 : parseInt(total['total_by_provider_by_user'][seat.provider_id]) + parseInt(1);
          total['total_by_type_by_user']['ticketing'] = total['total_by_type_by_user']['ticketing'] === undefined ? 1 : parseInt(total['total_by_type_by_user']['ticketing']) + parseInt(1);
          total['total_by_category_by_user'][seat.category_id] = total['total_by_category_by_user'][seat.category_id] === undefined ? 1 : parseInt(total['total_by_category_by_user'][seat.category_id]) + parseInt(1);
          total['total_by_type_price_by_user'][seat.type_price_id] = total['total_by_type_price_by_user'][seat.type_price_id] === undefined ? 1 : parseInt(total['total_by_type_price_by_user'][seat.type_price_id]) + parseInt(1);
          if (total['total_by_category_by_user_by_product'][seat.product_id] === undefined) {
            total['total_by_category_by_user_by_product'][seat.product_id] = [];
          }
          if (total['total_by_type_price_by_user_by_product'][seat.product_id] === undefined) {
            total['total_by_type_price_by_user_by_product'][seat.product_id] = [];
          }
          total['total_by_category_by_user_by_product'][seat.product_id][seat.category_id] = total['total_by_category_by_user_by_product'][seat.product_id][seat.category_id] === undefined ? 1 : parseInt(total['total_by_category_by_user_by_product'][seat.product_id][seat.category_id]) + parseInt(1);
          total['total_by_type_price_by_user_by_product'][seat.product_id][seat.type_price_id] = total['total_by_type_price_by_user_by_product'][seat.product_id][seat.type_price_id] === undefined ? 1 : parseInt(total['total_by_type_price_by_user_by_product'][seat.product_id][seat.type_price_id]) + parseInt(1);
  
          total['total_by_product_by_user'][seat.product_id] = total['total_by_product_by_user'][seat.product_id] === undefined ? 1 : parseInt(total['total_by_product_by_user'][seat.product_id]) + parseInt(1);
          total['total_by_variation_by_user'][seat.variation_id] = total['total_by_variation_by_user'][seat.variation_id] === undefined ? 1 : parseInt(total['total_by_variation_by_user'][seat.variation_id]) + parseInt(1);
  
  
          total['variation_label'][seat.variation_id] = seat.variation_name;
          total['product_label'][seat.product_id] = seat.product_name;
          total['category_label'][seat.category_id] = seat.category_name;
          total['type_price_label'][seat.type_price_id] = seat.type_price_name;
        }
      ...
      /**
       * Add ticket from the real cart in total virtual cart sales rule of type
       * rate
       *
       * @param {Array} totalInVSCart
       * @param {Array} cart
       * @returns {Array}
       */
      addTotalRateInRealCart: function (totalTypePricesInVSCart, id_product, cart) {
        if (
          cart !== undefined &&
          cart['attributes_by_product'] != undefined &&
          cart['attributes_by_product'][id_product] != undefined &&
          cart['attributes_by_product'][id_product]['attribute_type_price'] !== undefined
        ) {
          for (var type_price_id in cart['attributes_by_product'][id_product]['attribute_type_price']) {
            totalTypePricesInVSCart[type_price_id] === undefined ? cart['attributes_by_product'][id_product]['attribute_type_price'][type_price_id] : parseInt(totalTypePricesInVSCart[type_price_id]) + parseInt(cart['attributes_by_product'][id_product]['attribute_type_price'][type_price_id]);
          }
        }
        return totalTypePricesInVSCart;
      },
       ...
       * @returns {Array}
       */
      addPastOrderForSaleRule: function (totalTypePriceInVSCart, id_product, cart) {
        if (cart['attributes_by_product'] !== undefined && cart['attributes_by_product'][id_product] !== undefined) {
          if (cart['attributes_by_product'][id_product]['attribute_type_price'] !== undefined) {
            for (var type_price_id in cart['attributes_by_product'][id_product]['attribute_type_price']) {
              totalTypePriceInVSCart[type_price_id] = totalTypePriceInVSCart[type_price_id] === undefined ? cart['attributes_by_product'][id_product]['attribute_type_price'][type_price_id] : parseInt(totalTypePriceInVSCart[type_price_id]) + parseInt(cart['attributes_by_product'][id_product]['attribute_type_price'][type_price_id]);
            }
          }
        }
        return totalTypePriceInVSCart;
      },
  
      /**
       * Returns an error message if a sales rule is not valid
       *
       * @param {Integer} hvsc_id
       * @returns {String}
       */
      testIfVSCartIsAvailable: function (hvsc_id) {
  
        // Check if we already have an error message, we will return it
        // if there is no sale rule violation.
        $('.js-hvsc-vscart[hvsc-id=' + hvsc_id + ']').trigger('VSCartCheckErrorEvent', [hvsc_id]);
        var error = $('.js-hvsc-vscart[hvsc-id=' + hvsc_id + ']').prop('error');
  
        // don't check the sales rule for dispatched cart
        if (Drupal.behaviors.hubberVirtualSeatCart.hasDispatch()) {
          return error;
        }
  
        var cart = Drupal.behaviors.hubberVirtualCart.getVSCart(hvsc_id);
        var sale_rules = drupalSettings.saleRules;
        var sale_rules_order = drupalSettings.saleRuleOrder;
  
        if (sale_rules_order !== undefined) {
          var total_basket = this.getTotalInVSCart(cart);
          var msg = hubberSaleRulesProduct.testValidSaleRulesOrder(total_basket, sale_rules_order);
          if (msg !== '') {
            return msg;
          }
        }
        ...
        if (sale_rules !== undefined) {
          var total = this.getTotalInVSCart(cart, drupalSettings.dataBasketForSaleRules['has_sale_rule_by_user']);
          var msg = hubberSaleRulesProduct.testValidSaleRulesProduct(total, sale_rules, hvsc_id);
          if (msg !== '') {
            return msg;
          }
        }
      ...
      displayErrorMessage: function (hvsc_id) {
        $('.data-error-msg').remove();
        var message = this.testIfVSCartIsAvailable(hvsc_id);
        if (message == '' || message == undefined) {
          $('.js-hvsc-vscart .data-error-msg').remove();
          $('.js-hvsc-vscart .js-hvsc-vscart-btn-action').removeAttr('disabled');
          $('.js-hvsc-vscart .js-hvsc-vscart-btn-action').removeClass('js-hvsc-vscart-validate-not-available');
          $('.js-hvsc-vscart .js-hvsc-vscart-btn-action').addClass('js-hvsc-vscart-validate');
        }
        else {
          $('.js-hvsc-vscart .js-hvsc-vscart-btn-action').attr('disabled', 'disabled');
          $('.js-hvsc-vscart .js-hvsc-vscart-full .js-hvsc-vscart-contents-wrapper').prepend('<div class="data-error-msg"><span>' + message + '</span></div>');
          $('.js-hvsc-vscart .js-hvsc-vscart-btn-action').addClass('js-hvsc-vscart-validate-not-available');
          $('.js-hvsc-vscart .js-hvsc-vscart-btn-action').removeClass('js-hvsc-vscart-validate');
        }
      },
    }
  })(jQuery, Drupal, drupalSettings);
